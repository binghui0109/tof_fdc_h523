...
static uint32_t s_sum[ROWS][COLS];
static uint32_t s_sum_sq[ROWS][COLS];
static uint16_t s_valid_count[ROWS][COLS];
...
bool bg_collect(const VL53L5CX_ResultsData *tof_result)
{
    ...
    for (uint8_t row = 0U; row < ROWS; row++) {
        for (uint8_t col = 0U; col < COLS; col++) {
            uint8_t idx = (uint8_t)(row * COLS + col);
